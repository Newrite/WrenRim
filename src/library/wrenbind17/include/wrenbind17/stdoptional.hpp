...
namespace wrenbind17 {
    ...
    namespace detail {
        template <typename T> struct PushHelper<std::optional<T>> {
        ...
        template <typename T> struct PushHelper<const std::optional<T>&> {
            inline static void f(WrenVM* vm, int idx, const std::optional<T>& value) {
                PushHelper<std::optional<T>>::f(vm, idx, value);
            }
        };

        template <typename T> struct PopHelper<std::optional<T>> {
        ...
        template <typename T> struct PopHelper<const std::optional<T>&> {
            static inline std::optional<T> f(WrenVM* vm, const int idx) {
                return PopHelper<std::optional<T>>::f(vm, idx);
            }
        };
    } // namespace detail
#endif
